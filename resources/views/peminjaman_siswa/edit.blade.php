...
@section('content')
    ...
    <form method="POST" action="{{ route('peminjaman-siswa.update', $header->NoPinjamS) }}">
        @csrf @method('PUT')

        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label">No Pinjam</label>
                <input class="form-control" value="{{ $header->NoPinjamS }}" disabled>
            </div>
            <div class="col-md-6">
                <label class="form-label">No Anggota Siswa</label>
                <select name="NoAnggotaS" class="form-control" required>
                    @foreach ($anggota as $a)
                        <option value="{{ $a->NoAnggotaS }}" {{ $header->NoAnggotaS == $a->NoAnggotaS ? 'selected' : '' }}>
                            {{ $a->NoAnggotaS }} - {{ $a->NamaAnggota }}
                        </option>
                    @endforeach
                </select>
            </div>
                ...
                <tbody id="detailTableBody">
                    @foreach ($header->detail as $idx => $d)
                        <tr>
                            ...
                            <td><input name="detail[{{ $idx }}][Judul]" class="form-control" value="{{ $d->Judul }}" readonly>
                            </td>
                            <td><input name="detail[{{ $idx }}][Pengarang]" class="form-control" value="{{ $d->Pengarang }}"
                                    readonly></td>
                            <td><input name="detail[{{ $idx }}][Penerbit]" class="form-control" value="{{ $d->Penerbit }}"
                                    readonly></td>
        ...
        <div class="text-center mt-3">
            ...
            <a href="{{ route('peminjaman-siswa.index') }}" class="btn btn-secondary px-4">Kembali</a>
        </div>
